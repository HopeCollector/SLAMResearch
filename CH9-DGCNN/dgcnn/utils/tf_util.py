...
def pairwise_distance(point_cloud):
  ...
  # 忽略第一个轴，假设只有一幅点云参与计算计算
  # 尺寸为 point_cloud.shape = [N,3]
  ...
  # perm 参数表示对应的轴
  # 比如 0：第一轴，1：第二轴，2：第三轴
  # perm=[0,2,1] 就表示第一轴还在原来位置，只交换后两轴
  # perm=[2,1,0] 就表示第二轴不动，交换第一轴和第三轴
  # 这种广义层面的转置其实按列遍历数据并按行填充到新内存，只是填充好后会重新根据形状进行解释
  point_cloud_transpose = tf.transpose(point_cloud, perm=[0, 2, 1])
  point_cloud_inner = tf.matmul(point_cloud, point_cloud_transpose)
  # point_cloud_inner.shape = [N,N]
  # point_cloud_inner(i,j) = -2xixj -2yiyj -2zizj
  point_cloud_inner = -2*point_cloud_inner
  # axis 负数表示按照倒数第几个轴计算
  # point_cloud_square.shape = [N,1]
  # point_cloud_square[i,1] = xi^2 + yi^2 + zi^2
  point_cloud_square = tf.reduce_sum(tf.square(point_cloud), axis=-1, keep_dims=True)
  point_cloud_square_tranpose = tf.transpose(point_cloud_square, perm=[0, 2, 1])
  # ret.shape = [N,N]
  # ret(i,j) = xi^2 + yi^2 + zi^2 + xj^2 + yj^2 + zj^2 + -2xixj -2yiyj -2zizj
  #          = (pi-pj)^T @ (pi-pj)
...
def knn(adj_matrix, k=20):
  ...
  """
  # 因为 knn 找是最大值，所以这里要求距离最近的需要把距离变成负的
  neg_adj = -adj_matrix
...
def get_edge_feature(point_cloud, nn_idx, k=20):
  ...
  # <a,b>
  point_cloud_inner = tf.reduce_sum(tf.math.multiply(point_cloud_central, point_cloud_neighbors), axis=-1, keep_dims=True)
  # <a,a>
  point_cloud_central_len = tf.reduce_sum(tf.square(point_cloud_central), axis=-1, keep_dims=True)
  # <b,b>
  point_cloud_neighbors_len = tf.reduce_sum(tf.square(point_cloud_central), axis=-1, keep_dims=True)
  # sqrt(<a,a>)
  point_cloud_central_len = tf.math.sqrt(point_cloud_central_len)
  # sqrt(<b,b>)
  point_cloud_neighbors_len = tf.math.sqrt(point_cloud_neighbors_len)
  # <a,b> / sqrt(<a,a>)
  point_cloud_cos_theta = tf.math.divide(point_cloud_inner, point_cloud_central_len)
  # <a,b> / sqrt(<a,a>) / sqrt(<b,b>)
  point_cloud_cos_theta = tf.math.divide(point_cloud_cos_theta, point_cloud_neighbors_len)

  edge_feature = tf.concat([point_cloud_central, point_cloud_neighbors-point_cloud_central, point_cloud_cos_theta], axis=-1)
  # 返回值是一个 每行 k 个 [1,6] 的点对 （x_center, x_neighbor-x_center)
  # 其中第一个点是中心点，后面的都是邻近点
  return edge_feature
